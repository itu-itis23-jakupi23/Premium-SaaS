...
| RB-03 | GitHub CI clean checkout                  | in progress | Workflow parses clean (3 jobs, 18 verify steps) and now runs `public:metadata` and `public:routes`, which were previously local-only — a pull request could regress social metadata, browser zoom, or a legal route with CI green. Also fixed a diagnostic that could never fire: steps run under `bash -eo pipefail`, so the bare `node scripts/smoke-workflow.mjs` followed by `EXIT=$?` aborted before printing the api-server logs; guarded with `if !` and demonstrated in a shell. **Remote run still unverified** — requires pushing to GitHub. |
...
| WS-03 | Central transform precision and snap rules                      | implemented | `src/lib/workspace-transform.ts` is now the source of truth for the furniture grid (0.05 m), room grid (0.5 m), room dimension grid (1 m), corner snap (0.32 m), wall clearance (0.25 m), duplicate offset, and rounding precision; `PMWorkspace.tsx` consumes it. `booth-render.html` is self-contained and cannot import it, so `workspace:transform` asserts the renderer still encodes the same numbers and fails the gate on divergence — negative-tested on all four constants at exit 1. Renderer-side values remain literals: the gate enforces agreement, it does not remove the duplication. |
...
| ----- | -------------------------------------------------------------- | ----------- | --------------------------------------------------- | --------------------------------------------------------------------------------------------------------- |
| CS-01 | Remove staging `noindex, nofollow` and publish `robots.txt`     | in progress | `index.html:20`, `public/robots.txt`                | `robots.txt` exists and `public:routes` fails if it disagrees with the robots meta tag. **Blocked on a decision, not on work:** no canonical production domain is configured (`APP_URL` is localhost), and a `sitemap.xml` needs absolute URLs. Generating one against a guessed domain would publish wrong URLs to search engines, so it is deliberately not written. Provide the domain, then write the sitemap and flip both the meta tag and robots.txt together. |
...
| ID    | Requirement                                        | Status      | Relevant files                          | Verification target                                                                                    |
| ----- | -------------------------------------------------- | ----------- | --------------------------------------- | -------------------------------------------------------------------------------------------------------- |
| CS-14 | Marketing page has its own weight budget            | verified    | `scripts/check-frontend-budgets.mjs`    | The client portal serves the landing page at `/`, so it now carries a dedicated ratchet: 220 KiB gzip JS, 30 KiB gzip CSS, tighter than the general application budget. Currently 215.3 KiB / 27.9 KiB. Negative-tested: lowering the cap fails the gate at exit 1 with a named message. |
