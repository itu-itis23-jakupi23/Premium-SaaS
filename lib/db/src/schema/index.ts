...
export const exhibitionStatusEnum = pgEnum("exhibition_status", [
  "planned",
  "confirmed",
  "in_production",
  "on_site",
...
export const clients = pgTable(
  "clients",
  {
    ...
    leadValueCents: integer("lead_value_cents").notNull().default(0),
  ...
  (table) => ({
    ...
    orgLeadStageIdx: index("clients_org_lead_stage_idx").on(table.organizationId, table.leadStage),
    followUpIdx: index("clients_follow_up_idx").on(table.organizationId, table.nextFollowUpAt),
  }),
);
...
export const exhibitions = pgTable(
  "exhibitions",
  {
    ...
    standNumber: varchar("stand_number", { length: 60 }),
    status: exhibitionStatusEnum("status").notNull().default("planned"),
    opensAt: timestamp("opens_at", { withTimezone: true }),
    closesAt: timestamp("closes_at", { withTimezone: true }),
    moveInAt: timestamp("move_in_at", { withTimezone: true }),
    moveOutAt: timestamp("move_out_at", { withTimezone: true }),
    freightDeadlineAt: timestamp("freight_deadline_at", { withTimezone: true }),
    notes: text("notes"),
    metadata: jsonb("metadata").$type<Record<string, unknown>>().notNull().default({}),
    createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
    updatedAt: timestamp("updated_at", { withTimezone: true }).notNull().defaultNow(),
  ...
  (table) => ({
    orgOpensIdx: index("exhibitions_org_opens_idx").on(table.organizationId, table.opensAt),
    orgStatusIdx: index("exhibitions_org_status_idx").on(table.organizationId, table.status),
  }),
);
...
export const projects = pgTable(
  ...
  (table) => ({
    ...
    exhibitionIdx: index("projects_exhibition_idx").on(table.exhibitionId),
  }),
);
...
export const milestones = pgTable(
  "milestones",
  {
    ...
    /** Set when generated by the exhibition planner, so a re-plan updates in place. */
...
export const quotes = pgTable(
  ...
  (table) => ({
    orgNumberIdx: uniqueIndex("quotes_org_number_idx").on(table.organizationId, table.quoteNumber),
    ...
    projectIdx: index("quotes_project_idx").on(table.projectId),
  }),
);
